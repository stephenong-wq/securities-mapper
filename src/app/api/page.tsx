...
<div class="footer">This analysis is for internal advisory use only. Tax estimates are approximate and do not constitute tax advice.</div>
  ...
  const a = document.createElement("a")
  a.href = url
  a.download = `Transition-${(clientName || "Client").replace(/\s+/g, "-")}-${new Date().toISOString().slice(0,10)}.html`
                    ...
                    return (
                      <div key={group.assetClass} style={{ background: "var(--surface-raised)", border: `1px solid ${group.inTolerance ? "var(--border)" : "#ff448844"}`, borderRadius: 10, overflow: "hidden" }}>
                        ...
                        >
                          ...
                          {/* G/L $ */}
                          <span style={{ textAlign: "right", fontFamily: "var(--font-mono)", fontSize: 11, color: group.holdings.reduce((s,h)=>s+h.realizedGL,0) < 0 ? "#ff4488" : group.holdings.reduce((s,h)=>s+h.realizedGL,0) > 0 ? "#00f0c0" : "var(--ink-faint)" }}>
                            {group.holdings.reduce((s,h)=>s+h.realizedGL,0) !== 0 ? fmt$(group.holdings.reduce((s,h)=>s+h.realizedGL,0)) : "—"}
                          </span>
