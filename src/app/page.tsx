...
async function exportTransitionPDF(transition: TransitionSummary, trades: TradeRow[], clientName: string) {
  ...
  const name = clientName || transition.clientName || "Client"
...
export default function Home() {
  ...
  const handleTransitionImport = async (file: File) => {
    ...
    try {
      ...
      const data = await res.json()
      if (data.error) throw new Error(data.error)
      if (!data.transition) throw new Error("Server did not return transition data — check that route.ts is deployed")
