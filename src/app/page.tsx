...
// ─── CSV export ────────────────────────────────────────────────────────────────
function exportCSV(results: MappedSecurity[], modelLabel: string) {
  const rows = [["Input Ticker", "Status", "Mapped Ticker(s)", "Morningstar Style", "Asset Class", "Region", "Weight", "Notes"]]
  results.forEach(r => {
    if (r.mappings.length === 0) {
      rows.push([r.inputTicker, r.status, "", "", "", "", "", ""])
    } else {
      r.mappings.forEach((m, i) => {
        rows.push([
          i === 0 ? r.inputTicker : "",
          i === 0 ? r.status : "",
          m.ticker,
          ...
          m.assetClass,
          m.region,
          m.weight !== undefined ? `${Math.round(m.weight * 100)}%` : "",
          m.note || "",
        ])
      })
    }
  ...
  a.href = url
  a.download = `securities-mapping-${modelLabel.toLowerCase().replace(/\s+/g,"-")}-${new Date().toISOString().slice(0,10)}.csv`
  a.click()
}
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {dataLoading ? (
            ...
            <span style={{
              fontSize: 11, fontFamily: "var(--font-mono)", color: usingBlob ? "var(--green)" : "var(--amber)",
      ...
      <main style={{ flex: 1, maxWidth: 1100, width: "100%", margin: "0 auto", padding: "40px 40px 80px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "340px 1fr", gap: 32, alignItems: "start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div className="fade-up" style={{ background: "var(--surface-raised)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
              <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontFamily: "var(--font-display)", fontSize: 15, color: "var(--ink)" }}>Select Model</span>
              </div>
              <div style={{ padding: 8 }}>
                {MODELS.map(m => (
                  <button
                    key={m.id}
                    onClick={() => { setSelectedModel(m.id); setResults(null) }}
                    style={{
                  ...
                  >
                    ...
                    <div style={{ fontSize: 12, color: "var(--ink-faint)", lineHeight: 1.4 }}>
                      {m.description}
                    </div>
                  </button>
                ))}
          ...
          <div className="fade-up-2">
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, minHeight: 36 }}>
              ...
              {results && (
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  <button
                    onClick={() => exportCSV(results, selectedModelInfo.label)}
                    style={{
                      padding: "6px 14px", background: "var(--surface-raised)", border: "1px solid var(--border)",
                      borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: "pointer", color: "var(--ink)",
                    }}
                  >
                    ↓ Export CSV
                  </button>
            ...
            {!results && !loading && (
              <div style={{
                border: "1px dashed var(--border)", borderRadius: 12, padding: "80px 40px",
            ...
            {results && !loading && (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                {results.map((r, idx) => {
                  ...
                  const hasMultiple = r.mappings.length > 1

                  return (
                    <div
                      key={r.inputTicker + idx}
                      className={`fade-up`}
                      style={{
                        ...
                        return (
                          ...
                          >
                            <span style={{
                              ...
                              color: "var(--ink)", visibility: mi === 0 ? "visible" : "hidden",
                            }}>
                              {r.inputTicker}
                            </span>
                            <div>
                              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                ...
                                {m.weight !== undefined && (
                                  <span style={{
                                    ...
                                    background: "var(--split-light)", color: "var(--split-color)", fontWeight: 700,
                                  }}>
                                    {Math.round(m.weight * 100)}%
                                  </span>
                                )}
                              </div>
                              <div style={{ fontSize: 11, color: "var(--ink-faint)", marginTop: 2, maxWidth: 200 }} title={m.name}>
...
function Chip({ label, color, bg }: { label: string; color: string; bg: string }) {
  return (
    <span style={{ fontSize: 11, padding: "3px 9px", borderRadius: 20, background: bg, color, fontWeight: 700 }}>
      {label}
    </span>
  )
}
