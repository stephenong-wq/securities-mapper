...
export function mapSecurities(
  ...
  const mapped: (MappedSecurity | null)[] = inputTickers.map(raw => {
    ...
    if (!inputMs) return { inputTicker: ticker, status: "no-match" as const, mappings: [] }
    ...
    if (inputMs.splitRegions && inputMs.splitRegions.length > 0) {
      ...
      if (splitMappings.length > 0) return { inputTicker: ticker, status: "split" as const, mappings: splitMappings }
    }

    const candidates = findCandidates(msMap, universeForModel, inputMs.assetClass, inputMs.region)
    if (candidates.length === 0) return { inputTicker: ticker, status: "not-in-model" as const, mappings: [] }
